...
(function () {
  ...
  // Forked AMB to work from filesystem - depends on site-root-marker in navbar.html
  const searchDataURL = document.getElementById("site-root-marker").getAttribute("href") + "imerss-search-data.json";
  ...
  // AMB ADJUST
  function rebaseRoute(route) {
    const base = document.getElementById("site-root-marker").getAttribute("href");
    return base + route;
  }
  ...
  function displayResults(results, query) {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      let li = createElement(`
        <li>
          <a data-index="${i}" href="${rebaseRoute(result.route)}" class=${i === 0 ? "active" : ""}>
